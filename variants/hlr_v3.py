...
def hlr_v3(c0, c1, **kargs):
    ...
    subtasks = GridSubtasks(r_dim, non_goal_reward, goal_reward, c1, t_map)
    ...
    while n_samples < MAX_N_SAMPLES:
        ...
        pbar.set_description(f'alphas HL={HL.alpha:.4f} LL={subtasks.alpha:.4f}')

        while not terminate_episode:
            # Get a sample in the super-grid
            # Update the hihg-level function
            ...
            if next_state in HL.H[current_room]['exit_states']:

                hs = [current_room]
                hs.extend([h for h in HL.H if h is not current_room])

                for h in hs:
                    HL.update_exit_states_inside_room(h, subtasks.Z_i, subtasks.abstract_states)
